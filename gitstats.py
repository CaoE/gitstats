...
class GitDataCollector(DataCollector):
	def collect(self, dir):
		...
		for line in lines:
			...
			self.arthor_domain[author] = domain
			...
			if re.search('files? changed', line) == None:
				...
				if pos != -1:
					try:
						...
						domain = self.arthor_domain[author]
						# if domain == None:
						# 	raise 'this is none -----------------------------'
						if domain not in self.lines_added_by_domain_year:
							self.lines_added_by_domain_year[domain] = {}
						# if yy not in self.lines_added_by_domain_year[self.arthor_domain[author]]:
						# 	self.lines_added_by_domain_year[self.arthor_domain[author]] = {}
						self.lines_added_by_domain_year[domain][yy] = self.lines_added_by_domain_year[domain].get(yy, 0) + inserted
						if domain not in self.lines_removed_by_domain_year:
		...
		with open("/home/ecao/utils/gitstats/result/domain_years.json", 'w') as f:
			...
			f.write(str(self.lines_added_by_domain_year))
			f.write("*"*50)
		...
		# print(self.lines_removed_by_domain_year)
		# print("*"*50)
		# print("lines_removed_by_domain_year")
		# print(self.lines_removed_by_domain_year)
		...
		# #getpipeoutput(['git log --shortstat %s --pretty=format:"%%at %%aN" %s' % (extra, getlogrange('HEAD'))]).split('\n')
			...
			if re.search('files? changed', line) == None:
				...
				if pos != -1:
					try:
						...
						date = datetime.datetime.fromtimestamp(stamp)
						yy = date.year
						if self.arthor_domain[author] not in self.lines_added_by_domain_year2:
							self.lines_added_by_domain_year2[self.arthor_domain[author]] = {}
						# if yy not in self.lines_added_by_domain_year[self.arthor_domain[author]]:
						# 	self.lines_added_by_domain_year[self.arthor_domain[author]] = {}
						self.lines_added_by_domain_year2[self.arthor_domain[author]][yy] = self.lines_added_by_domain_year2[self.arthor_domain[author]].get(yy, 0) + inserted
						if self.arthor_domain[author] not in self.lines_removed_by_domain_year2:
		...
		with open("/home/ecao/utils/gitstats/result/domain_years2.json", 'w') as f:
			...
			f.write(str(self.lines_added_by_domain_year2))
			f.write("*"*50)
